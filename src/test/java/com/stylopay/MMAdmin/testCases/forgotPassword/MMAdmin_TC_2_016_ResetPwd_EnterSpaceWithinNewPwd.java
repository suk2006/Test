...
public class MMAdmin_TC_2_016_ResetPwd_EnterSpaceWithinNewPwd extends BaseClass{
	
	String password;

	@Test(groups = { "forgotPwd", "sanity" })
	public void MMAdmin_TC_2_016() throws IOException, InterruptedException {

		Reporting.startTest("MMAdmin_TC_2_016");
		...
		try {
			...
			forgotPasswordPage.enterEmail(
					ReadTestData.readTestDataFromExcel("ForgotPassword", "Sheet1", "MMAdmin_TC_2_016", "email"));
			forgotPasswordPage.clkSubmitBtn();
			WaitFunctions.threadWait();

			ResetPassword resetPwd = new ResetPassword(driver);

			resetPwd.enterOTP(ReadTestData.readTestDataFromExcel("ResetPassword", "Sheet1", "MMAdmin_TC_2_016", "OTP"));

			password = ReadTestData.readTestDataFromExcel("ResetPassword", "Sheet1", "MMAdmin_TC_2_016",
					"New Password");
			resetPwd.enterNewPwd(password);
			resetPwd.enterConfirmPwd(ReadTestData.readTestDataFromExcel("ResetPassword", "Sheet1", "MMAdmin_TC_2_016",
					"Confirm Password"));
			...
			try {
				...
				if (resetPwd.validatePasswordFormat(password) == false) {
					...
					logger.info("Password validation is working if spaces is entered within password");
					Reporting.test.log(LogStatus.PASS, "Enter space within password",
							"Password validation is working if spaces is entered within password");

				} else {

					logger.error("Password validation is not working if spaces is entered within password");
					Reporting.test.log(LogStatus.FAIL, "Enter space within password",
							"Password validation is not working if spaces is entered within password"
									+ Reporting.test.addScreenCapture(captureScreen(driver, "MMAdmin_TC_2_016")));
				}

				if (ReadExpectedValidationAndSuccessMsgFromXL.getTestReportDataFromExcel("ResetPassword", "Sheet1",
						"MMAdmin_TC_2_016", "Password Format Validation Err Msg")
				...
				logger.error("Error message: Password validation is not working if space in entered within password");
				Reporting.test.log(LogStatus.FAIL, "Enter space within password",
						"Error message: Password validation is not working if space in entered within password"
								+ Reporting.test.addScreenCapture(captureScreen(driver, "MMAdmin_TC_2_016")));
			}
